<template>
  ...
  <el-select v-model.number="defaultParams.button" class="mt-2 w-full" label="Button">
    <el-option v-for="button in buttons" :key="button.id" :value="button.id" :label="button.name"></el-option>
  </el-select>
  <div v-for="items in posGroups" :key="items[0]" class="flex items-center space-x-2 mt-2">
    <el-input
