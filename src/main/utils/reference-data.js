...
import { isObject, objectHasKey, replaceMustache } from './helper'
...
export function parseKey(key) {
  ...
  if (['prevBlockData', 'loopData', 'globalData', 'activeTabUrl'].includes(dataKey)) return { dataKey, path: path || '' }
...
export default function (block, data) {
  const replaceKeys = ['url', 'name', 'body', 'value', 'fileName', 'selector', 'prefixText', 'suffixText']
  let replacedBlock = block

  replaceKeys.forEach(blockDataKey => {
    if (!objectHasKey(block.data, blockDataKey)) return

    const newDataValue = replaceMustache(replacedBlock.data[blockDataKey], match => {
      ...
      if (dataKey === 'prevBlockData' && (!isObject(data.prevBlockData) || !Array.isArray(data.prevBlockData))) {
    ...
    replacedBlock = objectPath.set(replacedBlock, `data.${blockDataKey}`, newDataValue)
  })
