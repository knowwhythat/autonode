...
const renderContent = (content, contentType) => {
  ...
  if (contentType === 'form') {
    return Object.keys(renderedJson)
      .map(key => {
        const value = isObject(renderedJson[key]) ? JSON.stringify(renderedJson[key]) : renderedJson[key]
...
const convertContentType = contentType => {
  return contentType === 'json' ? 'application/json' : 'application/x-www-form-urlencoded'
}

export async function executeWebhook({ url, contentType, headers, timeout, body }) {
